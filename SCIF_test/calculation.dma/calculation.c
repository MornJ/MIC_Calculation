...
//        }
        ...
        int volatile *p = (int volatile *)recvdata;
        while (*p == 0) usleep(100);
        length = *p;
        if (length == -1)
            goto close;
        err = 0;
        length = length - sizeof(instr_info_buffer_t);
        instr_info_buffer_t* copydata = (instr_info_buffer_t*)malloc(length);
        memcpy(copydata, recvdata+1, length);
//        if (ag_length != length){
//            printf("ag %d length %d\n",ag_length, length);
//        }
...
//        }
        //printf("err = %d\n", err);
        send_sig(tid, copydata, length/ sizeof(instr_info_buffer_t)-1, skip_count);
        *p = 0;
        //free(copydata);
...
close:
    ...
    printf("Connection is complete, skip %d, ",*skip_count);
    free(skip_count);
    fflush(stdout);
