...
void* listen_msg(void* vargp){
    ...
    uint64_t* buffer = ((transfer_data_t*)vargp)->buffer;
    ...
    if ((tid-1) % TRANSFER_BUNCH == 0){
        ...
        buffer_size = 8*SIGNAL_NUM+THRESHOLD*QUEUE_SIZE*(sizeof(uint64_t)*2) + NEW_ENTRY*sizeof(instr_info_buffer_t);
    ...
    uint64_t *remote_cur = (uint64_t *)((char*)remote_ptr[bid]+offset) + 1;
    ...
    *cur = QUEUE_SIZE-1;
    ...
    uint64_t volatile *ent = (uint64_t volatile *)recvdata + 3;
    *ent = 0;
    
    uint64_t* addr_queue = (uint64_t*)recvdata + SIGNAL_NUM;
    instr_info_buffer_t* entries= (instr_info_buffer_t*)((char*)recvdata + sizeof(uint64_t)*2*THRESHOLD*QUEUE_SIZE+SIGNAL_NUM*8);
...
close:
    ...
    if (tid == 1){
        ...
        for (int j = 0; j < t_b; j++){
            uint64_t* c_buffer = buffers[j];
            for (int i = 0; i< TRANSFER_BUNCH;i++){
                *(uint64_t*)((char*)c_buffer + i*buffer_size) = 0xFFFF;
            }
    ...
    if (err)
        printf("Test failed\n");
    printf("total_time %.2f wait %.2f cal %.2f write %.2f in thread %d\n", 
            total_timeuse, timeuse, c_time,t_times[0], tid);
    return NULL;
...
//    cpu_set_t get;
    ...
    int buffer_size = 8*SIGNAL_NUM+THRESHOLD*QUEUE_SIZE*(sizeof(uint64_t)*2) + 
        NEW_ENTRY*sizeof(instr_info_buffer_t);
    int len = buffer_size*TRANSFER_BUNCH;
    len = (len / pagesize+1) * pagesize;
    printf("len %d page %d instr %d int %d\n", len, pagesize,
            sizeof(instr_info_buffer_t), sizeof(int));
    buffers = (uint64_t**)malloc(sizeof(uint64_t*)*bunch); 
...
//        CPU_SET(i%60, &mask);
        ...
        t->threadid = i;
        t->buffer = (uint64_t*)((char*)(buffers[no])+offset*buffer_size);
